...
public:
    bool helper(vector<int>&nums, int ind,vector<int> &dp){
        ...
        if(ind+2<n and nums[ind]==nums[ind+2] and nums[ind]==nums[ind+1]){
            if(helper(nums,ind+3,dp))return dp[ind]=true;
        }
        if(ind+2<n and nums[ind]+2==nums[ind+2] and nums[ind]+1==nums[ind+1]){
            if(helper(nums,ind+3,dp))return dp[ind]=true;
        }
        return dp[ind]=false;
    }
...
//         if (i + 1 < n && v[i] == v[i + 1]){
//             if (c(v, dp, i + 2))return dp[i] = true;
//         }
//         if (i + 2 < n && v[i] == v[i + 2] and v[i] == v[i + 1]){
//             if (c(v, dp, i + 3))return dp[i] = true;
//         }
